 ...
 *
 * The followings are the available columns in table 'activerecord_log':
 * @property integer $id
...
class ActiverecordLog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('action, model, field, ip', 'length', 'max'=>20),
			array('idUser', 'length', 'max'=>15),
			array('socnr', 'length', 'max'=>4),
			array('description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, description, action, model, idModel, field, ip, idUser, socnr', 'safe', 'on'=>'search'),
		);
	}
