...
class Catalogos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('tipo, idCat, refnr', 'length', 'max'=>20),
			array('descripcion', 'length', 'max'=>50),
			array('estado', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, tipo, idCat, descripcion, refnr, estado', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('idCat',$this->idCat,true);
		$criteria->compare('descripcion',$this->descripcion,true);
		$criteria->compare('refnr',$this->refnr,true);
